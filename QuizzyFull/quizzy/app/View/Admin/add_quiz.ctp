...
<script type="text/javascript">
	$(function() {
		...
		$('#btnSave').click(function() {
			...
			if( $('#selType').val() == "Choose") {
				if( prevQuiz == '' ) {
					alert("Nothing to save, empty quiz.");
					return;
				}
				
				var tmpResp = confirm("Are you sure you want to save without adding a question?");
				if( tmpResp == false )
					return;
			}
		...
		$('#selCheckboxCount, #selRadioCount, #selSelectCount').change( function() {
			...
			for( var i = 0; i < tmpCount; i++ ) {
				...
				if( $(this).attr('id') == "selRadioCount" ) {
					strAdd = '<div class="addItem">Option '+(i+1).toString()+' <input type="textbox" id="txtTextboxVal" value="Value" /></div>';
					$('#addRadioList').append(strAdd);
				} else if( $(this).attr('id') == "selSelectCount" ) {
					strAdd = '<div class="addItem">Option '+(i+1).toString()+' <input type="textbox" id="txtTextboxVal" value="Value" /></div>';
					$('#addSelectList').append(strAdd);
				} else {
					strAdd = '<div class="addItem">Option '+(i+1).toString()+' <input type="textbox" id="txtCheckboxVal" value="Value" /></div>';
		...
		$('#btnDoMatrix').on('click', function() {
			...
			for( var i = 0; i < intMatRow ; i++ ) {
				var strAdd = '<div class="matrixQst">Question '+(i+1).toString()+' <input type="textbox" id="txtQText" value="Question'+(i+1).toString()+'" /></div>';
				$('#addMatrixInnerRows').append(strAdd);
			}
			for( var i = 0; i < intMatCol ; i++ ) {
				var strAdd = '<div class="matrixOpt">Option '+(i+1).toString()+' <input type="textbox" id="txtOText" value="Option'+(i+1).toString()+'" /></div>';
...
<div id="buildQuiz" style="width: 50%;">
	...
	<table>
		<tr>
			<td>Quiz title:</td><td><input type="textbox" id="fldQuizTitle" name="fldQuizTitle" value="This is the title" /></td>
		</tr>
		...
		<tr>
			<td>Question Type:</td><td>
				<select id="selType">
				  <option value="Choose">Please choose a field</option>
		...
		<tr>
			<td>Image:</td><td>
				<input type="checkbox" name="chkPicture" id="chkPicture">Include?
		...
		<tr class="trQuestionText" style="display: none;">
			<td>The question text:</td><td>
			<div id="divQuestionText" style="display: none;">
				<input type="textbox" id="txtQuestionText" value="Question" />
			</div>
			</td>
		</tr>
	</table>
	...
	<table>
		<tr id="addCheckbox" class="classAdd">
			<td colspan="2">
				<table>
					<form>
						<tr><td>How many checkboxes?</td><td>
						...
						<tr><td colspan="2">
							<div id="addCheckboxList"> 
								...
								<div class="addItem">Option 1 
									<input type="textbox" id="txtCheckboxVal" value="Value" />
								</div>
							</div>
						</td></tr>
					</form>
					...
					<form>
						...
						<tr><td colspan="2">
							<div id="addRadioList"> 
								<!-- has one for the first add :) -->
								<div class="addItem">Option 1 
									<input type="textbox" id="txtTextboxVal" value="Value" />
								</div>
							</div>
						</td></tr>
					</form>
		...
		<tr id="addMatrix" class="classAdd">
			<td colspan="2">
				<table>
					<form>
						<tr><td>How many lines?</td><td><input type="textbox" id="txtMatrixLines" value="" /></td></tr>
						<tr><td>How many cols? </td><td><input type="textbox" id="txtMatrixCols" value="" /> </td></tr>
						<tr><td> </td><td><input type="button" id="btnDoMatrix" value="Create!" /></td></tr>
						<tr><td colspan="2">
		...
		<tr><td><input type="button" id="btnAdd" value="Add" /></td>
		    <td><input type="button" id="btnSave" value="Save" /></td></tr>
	</table>
	</form>
</div>
