...
<h2>שאלון</h2>
<h3>ברוכים הבאים ל'<?php echo $patQuiz['Quiz']['quizTitle']; ?>'</h3>
...
<?php
	...
	if( count($patQuiz['Questions']) < 1 ) {
		echo '<h2>שגיאה: לשאלון לא הוגדרו שאלות!</h2>';
	}
	...
	if( $flagMatrix != 0 )
		printMatrixClose();
?>
<input type="submit" id="btnSubmit" value="שמירה!" />
</form>
<div id="divData"></div>
...
function printQuestion( $qD ) {
	...
	switch ( $qD['questionType'] ) {
		...
		case "SELECT":
			...
			echo '<option value="!EMPTY!">אנא בחרו:</option>'."\n";
			foreach ( $tempOptions as $currOpt ) {
				if( strlen( $currOpt ) < 1 )
					continue;
